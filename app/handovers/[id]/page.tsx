...
export default function HandoverDetailPage() {
  ...
  const fetchHandoverDetails = async (page: number = currentPage, resetPage: boolean = false, isFilterChange: boolean = false) => {
    ...
    try {
      ...
      setError('Error fetching handover details');
    } finally {
      setIsLoading(false);
  ...
  if (!handover) {
    return (
      <div className="min-h-full bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800 p-6">
        <div className="text-center py-12">
          <h1 className="text-2xl font-bold text-gray-800 dark:text-white mb-4">Handover Not Found</h1>
          <p className="text-gray-600 dark:text-gray-400">The requested handover could not be found.</p>
        </div>
      </div>
    );
  }
  ...
  const handleDeleteHandover = async () => {
    ...
    try {
      ...
      if (data.success) {
        alert(data.message || 'Handover deleted successfully');
        router.push('/handovers'); // Redirect to handovers list
      } else {
        alert(data.error || 'Failed to delete handover');
      }
    } catch (error) {
  ...
  return (
    <div className="min-h-full bg-gradient-to-br from-slate-50 to-blue-50 dark:from-slate-900 dark:to-slate-800 p-6 space-y-6">
          ...
          <div className="bg-white dark:bg-slate-800 rounded-lg max-w-md w-full p-6">
            ...
            <div className="mb-4">
              <p className="text-sm text-slate-600 dark:text-slate-400">
                Are you sure you want to delete this handover? This action will also delete all {handover?.totalParcels || 0} associated parcels and cannot be undone.
              </p>
              <div className="mt-2 p-3 bg-red-50 dark:bg-red-900/20 rounded-md">
                <p className="text-sm text-red-800 dark:text-red-400 font-medium">
                  Handover: {handover?.file_name}
                </p>
                <p className="text-sm text-red-600 dark:text-red-400">
                  Date: {handover && new Date(handover.handover_date).toLocaleDateString()}
                </p>
              </div>
            </div>
            ...
            <div className="flex justify-end space-x-3">
              <button
                ...
                disabled={isDeleting}
                className="px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-300 bg-white dark:bg-slate-700 border border-slate-300 dark:border-slate-600 rounded-md hover:bg-slate-50 dark:hover:bg-slate-600 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50"
              >
                Cancel
              </button>
              <button
                ...
                disabled={isDeleting}
                className="px-4 py-2 text-sm font-medium text-white bg-red-600 hover:bg-red-700 rounded-md focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-red-500 disabled:opacity-50 disabled:cursor-not-allowed flex items-center"
              >
                {isDeleting ? (
                  <>
      ...
      <div className="flex gap-3">
        {/* Print Report Button - doesn't need modal props */}
        <PrintHandoverReport handoverId={handover.id} />
